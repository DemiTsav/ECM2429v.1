...
class AssetManagementPage(tk.Frame):
    ...
    def _create_vehicle_table(self) -> None:
        ...
        self.scrollbar = self._add_scrollbar(self.vehicle_table_frame, self.vehicle_table)
    ...
    def _create_action_buttons(self) -> None:
        ...
        buttons = [
            ...
            ("Create a Report", lambda: self.open_new_window(self.open_asset_reporting_page, self.db)),
        ]

        for text, command in buttons:
            UIComponents.create_button(self, self.actions_frame, text=text, command=command, padx=10)
    ...
    def show_all_vehicles(self) -> None:
        """Reset the vehicle table and display all vehicles from the database."""
    ...
    def _create_vehicle_form(self, action: str, process_callback: callable) -> None:
        tk.Label(self.dynamic_content_frame, text=f"{action} Form", font=("Helvetica", 14)).pack(pady=5)
        fields = ["Make", "Model", "Year", "Vehicle Type", "Fuel Type", "Service Date", "Tax Due Date"]
        ...
        tk.Button(
            ...
            command=lambda: process_callback(self.entries, self.tax_status_dropdown.get())
        ).pack(pady=10)
    ...
    def _create_tax_status_dropdown(self) -> None:
        ...
        self.tax_status_dropdown = ttk.Combobox(
            self.dynamic_content_frame, values=["Tax Paid", "Tax Due", "SORN", "Exempt"]
        )
        self.tax_status_dropdown.pack(fill="x", pady=2)

    def _process_add_vehicle(self, entries: dict, tax_status: str) -> None:
        VehicleProcess.process_add_vehicle(self, entries, self.tax_status_dropdown)
    ...
    def show_update_vehicle_form(self) -> None:
        ...
        fields: list = ["Make", "Model", "Year", "Vehicle Type", "Fuel Type", "Service Date", "Tax Due Date"]
        self.update_entries = {}

        for field in fields:
            UIComponents.checkbox_updates(self, field, vehicle_info)

        tax_status_var, tax_status_dropdown = UIComponents.create_tax_status_dropdown(self, vehicle_info)
    ...
    def delete_vehicle_prompt(self) -> None:
        self.clear_dynamic_content()
        self.show_all_vehicles()
        tk.Label(self.dynamic_content_frame, text="Please select a vehicle to delete.", fg="blue").pack()
        UIComponents.create_button(self, self.dynamic_content_frame, text="Delete", command= self.confirm_deletion, padx=10)
    ...
    def confirm_deletion(self): 
        ...
        delete_button = tk.Button(
            ...
            command=VehicleProcess.show_deletion_confirmation(self, selected_items),
    ...
    def show_search_form(self) -> None:
        ...
        fields: list = ["Make", "Model", "Year", "Vehicle Type", "Fuel Type", "Service Date", "Tax Due Date"]
        self.search_entries = {}  # Store entries as an instance variable for reuse
        ...
        for field in fields:
            ...
            entry.bind("<KeyRelease>", lambda event: VehicleProcess.perform_search(self))  # Dynamic update on key release
        ...
        tk.Label(frame, text="Tax Status", width=15, anchor="w").pack(side="left")
        ...
        tax_status_dropdown.bind("<<ComboboxSelected>>", lambda event: VehicleProcess.perform_search(self))  # Dynamic update on selection
        self.search_entries["Tax Status"] = tax_status_dropdown
        VehicleProcess.perform_search(self)  # Perform search on initial load
    ...
    def open_asset_reporting_page(self, new_window: tk.Toplevel, db: VehicleDatabase) -> None:
