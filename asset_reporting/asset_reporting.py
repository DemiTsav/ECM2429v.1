...
class AssetReportingPage(tk.Frame):
    ...
    def create_widgets(self):
        ...
        report_options = ["All Vehicles", "Vehicles Due for Service", "Vehicles with Tax Due",
        ...
        self.vehicles = []  # Store vehicles data to be exported
    ...
    def display_report(self):
        ...
        report_header = "ID | Make | Model | Year | Vehicle Type | Fuel Type | Service Date | Tax Due Date | Tax Status\n"
        report_separator = "-" * 90
        self.report_text.insert(tk.END, report_header + report_separator + "\n")

        for vehicle in self.vehicles:
            vehicle_info = f"{vehicle[0]} | {vehicle[1]} | {vehicle[2]} | {vehicle[3]} | {vehicle[4]} | {vehicle[5]} | {vehicle[6]} | {vehicle[7]} | {vehicle[8]}\n"
    ...
    def custom_report(self):
        ...
        self.filters = {}
        self.filters["Make"] = self.create_filter(self.filter_frame, "Make")
        self.filters["Model"] = self.create_filter(self.filter_frame, "Model")
        self.filters["Year"] = self.create_filter(self.filter_frame, "Year")
        self.filters["Fuel Type"] = self.create_filter(self.filter_frame, "Fuel Type")
        self.filters["Vehicle Type"] = self.create_filter(self.filter_frame, "Vehicle Type")
        self.filters["Tax Status"] = self.create_filter(self.filter_frame, "Tax Status")
        self.filters["Tax Due Date"] = self.create_filter(self.filter_frame, "Tax Due Date")
        self.filters["Service Date"] = self.create_filter(self.filter_frame, "Service Date")
    ...
    def create_filter(self, window, label_text):
