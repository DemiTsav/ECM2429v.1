...
# Test the report generation UI functionality
def test_generate_report(temp_db, mocker):
    # Mock the AssetReportingHandler to avoid actual database queries
    ...
    app.report_var.set("All Vehicles")  # Set report type
    app.generate_report()
...
# Test the custom report generation functionality
def test_generate_custom_report(temp_db, mocker):
    # Mock the custom report fetch functionality
...
def test_export_to_csv(temp_db, mocker):
    ...
    # Create the tkinter window and AssetReportingPage instance
    root = tk.Tk()
    app = AssetReportingPage(root, temp_db)
    app.vehicles = [
        (1, 'ABC123', 'Toyota', 'Corolla', 2015, 'Sedan', 'Petrol',
    ...
    app.export_to_csv()  # Call the export function

    # Check if the save file dialog was triggered
    mock_asksaveasfilename.assert_called_once_with(defaultextension=".csv",
                                                   filetypes=[(
                                                       "CSV Files", "*.csv"
                                                       )])

    # Check if the messagebox was shown for success
